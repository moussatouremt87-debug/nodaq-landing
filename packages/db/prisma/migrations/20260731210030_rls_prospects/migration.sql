-- ============================================================================
-- Row-Level Security sur `prospects` et `prospect_interactions` (ticket 2.12 —
-- CRM & prospection : données personnelles de TIERS non clients).
--
...
ALTER TABLE "prospect_interactions" FORCE ROW LEVEL SECURITY;

CREATE POLICY tenant_isolation ON "prospect_interactions"
  FOR ALL
  USING (tenant_id = NULLIF(current_setting('app.current_tenant_id', TRUE), '')::uuid)
  WITH CHECK (tenant_id = NULLIF(current_setting('app.current_tenant_id', TRUE), '')::uuid);
